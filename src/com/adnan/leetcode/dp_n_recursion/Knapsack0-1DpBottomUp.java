...
public class Knapsack0-1DpBottomUp {
    ...
    public static int knapsack(int val[], int wt[], int W) {
        ...
        for (int item = 1;item <= N; item++) {
            ...
            for (int weight = 1; weight <= W; weight++){
                ...
                if (wt[item-1] <= weight) {
                    // Given a weight, check if the value of the current item + value of
                    // the item that we could afford (more) with the remaining weight is
                    // greater than the value without the current item itself

                    int t1 = V[item-1][weight-wt[item-1]]; // if repeatition is not allowed
                    // int t2 = V[item][weight-wt[item-1]]; // if repeatition is allowed
                    V[item][weight] = Math.max(val[item-1] + t1, V[item-1][weight]);
                }
                else {
